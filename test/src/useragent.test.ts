...
describe("UserAgentComposer", () => {
  it("initializes with local MCP agent string", () => {
    ...
    expect(sut.userAgent).toBe("AzureDevOps.MCPSafe/1.0.0 (local)");
  });
  ...
  it("appends MCP client info once", () => {
    ...
    expect(sut.userAgent).toBe("AzureDevOps.MCPSafe/1.0.0 (local) Jest/0.0.1");
  ...
  it("ignores incomplete MCP client info", () => {
    ...
    sut.appendMcpClientInfo({ name: "Node", version: "22.0.0" });
    expect(sut.userAgent).toBe("AzureDevOps.MCPSafe/1.0.0 (local) Node/22.0.0");
  });
});
